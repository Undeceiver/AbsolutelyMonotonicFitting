...
We discuss the problem of fitting absolutely monotonic functions to noisy random samples in bounded intervals. This specific shape-constrained estimation problem has not been significantly explored in the literature, though it is related to other kinds of shape-constrained estimation such as fitting exponential sums or convex polynomial fitting. A wide variety of potential approaches applicable to the problem are discussed, including various forms of exponential sum fitting, using Bernstein polynomial bases or splines in constrained optimization problems, and non-linear least squares optimization with Vandermonde matrices. A novel approach, Greedy Lowballing Absolutely Monotonic Polynomial Fitting (GLAMPF) is also introduced. This method finds monomial terms to iteratively approximate the sampled data from below (lowballing), ensuring absolute monotonicity by construction and robustness to a variety of function shapes. However, it lacks general optimality and occasionally underestimates certain regions of the sample. A few of the most promising methods were implemented and compared on generated samples from a wide variety of absolutely monotonic and convex functions. While convex splines displayed the best results in cases where low degree polynomials were sufficient, GLAMPF was the most robust approach with the strongest guarantees and produced acceptable results in every test case.
